...
class EntityA:
    ...
    def __init__(self, seqnum_limit):
        ...
        self.queue = []
        self.buffer = []
        self.packetInRoute = False

    # Called from layer 5, passed the data to be sent to other side.
    # The argument `message` is a Msg containing the data to be sent.
    def output(self, message):
        #Create packet to be added to buffer or to be sent
        checksum = createCheckSum(self.seqnum, self.acknum, message.data)
        packet = Pkt(self.seqnum, self.acknum, checksum, message.data)
        ...
        #Check if we have a packet in route if so append to buffer
        #else we send the packet down to layer 3
        if(self.packetInRoute):
            self.buffer.append(packet)
        else:
            #Add packet to copy queue in case it needs to be resent
            self.acknum = packet.seqnum
            self.queue.append(packet)

            #Send created packet down to layer 3
            to_layer3(self, packet)
            self.packetInRoute = True
            start_timer(self, 10)
    ...
    def input(self, packet):
        ...
        if(packet.checksum == createCheckSum(packet.seqnum, packet.acknum, packet.payload)):
            #Check the length of the queue and see if the packet acknum is the same as the sent packet
            if (len(self.queue) > 0 and self.queue[0].seqnum == packet.acknum):
                stop_timer(self)
                self.packetInRoute = False
                ...
                #Check to see if the buffer has items to be sent
                if(len(self.buffer) > 0):
                    #Grab packet from beginning of buffer
                    packet = self.buffer.pop(0)
                    self.acknum = packet.seqnum

                    #Send packet to layer 3 to start TCP process
                    to_layer3(self, packet)
                    self.packetInRoute = True
                    self.queue.append(packet)
                    start_timer(self, 10)
    ...
    def timer_interrupt(self):
        #Grab last packet and send it down to layer 3 and restart timer.
        packet = self.queue[0]
        to_layer3(self, packet)
        start_timer(self, 10)
...
class EntityB:
    ...
    def input(self, packet):
        ...
        if(packet.checksum == createCheckSum(packet.seqnum, packet.acknum, packet.payload) and self.expectedSeqnum == packet.seqnum):
            ...
            ackPacket = Pkt(packet.seqnum, packet.seqnum, createCheckSum(packet.seqnum, packet.seqnum, packet.payload), packet.payload)
            to_layer3(self, ackPacket)

            #Alternate our expected seqnum upon receiving (alternating bit)
            if(packet.seqnum == 1):
                self.expectedSeqnum = 0
            else:
                self.expectedSeqnum = 1
        #Else we received a duplicate packet and we need to let Entity A know
        elif packet.checksum == createCheckSum(packet.seqnum, packet.acknum, packet.payload):
            ackPacket = Pkt(packet.seqnum, packet.seqnum, createCheckSum(packet.seqnum, packet.seqnum, packet.payload), packet.payload)
            to_layer3(self, ackPacket)
