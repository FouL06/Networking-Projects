...
Class: CS 4480 Spring 2022
Version: 3/28/22 v0.02
"""
...
class EntityA:
    ...
    def __init__(self, seqnum_limit):
        ...
        self.seqnumLimit = seqnum_limit

    # Called from layer 5, passed the data to be sent to other side.
    # The argument `message` is a Msg containing the data to be sent.
    def output(self, message):
        # Check if window has space and send the packet
        if len(self.sentPackets) < self.windowSize:
            # Create Packet and Checksum
            checksum = createCheckSum(
                self.expectedSeqnum, self.expectedSeqnum, message.data)
            packet = Pkt(self.expectedSeqnum, self.expectedSeqnum,
                         checksum, message.data)
            self.sentPackets.append(packet)
            to_layer3(self, packet)

            # Check if base is equal to seqnum and start timer
            if self.base == self.expectedSeqnum:
                start_timer(self, 10 + (4 * self.windowSize))

            # Iterate expectedSequnum
            if self.expectedSeqnum == self.seqnumLimit - 1:
    ...
    def input(self, packet):
        # Check if packet we received is not corrupted else we resend the
        if checkNotCorrupt(packet.checksum, packet.seqnum, packet.acknum, packet.payload):
            acknumInWindow = False

            #Check to see if the acknum is within the current window size
            if self.expectedSeqnum < self.base:
                if packet.acknum >= self.base or packet.acknum < self.expectedSeqnum:
                    acknumInWindow = True
            elif packet.acknum >= self.base and packet.acknum < self.expectedSeqnum:
                acknumInWindow = True
            if not acknumInWindow:
                return

            #Remove packets that should be acknowledged
            while len(self.sentPackets) > 0:
                removedPacket = self.sentPackets.pop(0)
                if removedPacket.seqnum == packet.acknum:
                    break

            #Wrap around for window
            self.base = packet.acknum + 1
            if self.base >= self.seqnumLimit:
                self.base -= self.seqnumLimit

            #Restart timer for resent ack-packets
            if self.base == self.expectedSeqnum:
                stop_timer(self)
            else:
                stop_timer(self)
    ...
    def timer_interrupt(self):
        print("A - recovering from lost packet")
        # Restart timer and resend un-acked packets
        if len(self.sentPackets) > 0:
            start_timer(self, 10 + (4 * self.windowSize))
            for i in range(len(self.sentPackets)):
                to_layer3(self, self.sentPackets[i])
...
class EntityB:
    ...
    def __init__(self, seqnum_limit):
        ...
        self.seqnumLimit = seqnum_limit
        self.windowSize = seqnum_limit // 2
        self.sndpkt = Pkt(0, 0, 420, bytes(Msg.MSG_SIZE))

    # Called from layer 3, when a packet arrives for layer 4 at EntityB.
    # The argument `packet` is a Pkt containing the newly arrived packet.
    def input(self, packet):
        # Check if packet is corrupted and our expected sequence numbers are the same
        if(checkNotCorrupt(packet.checksum, packet.seqnum, packet.acknum, packet.payload) and self.expectedSeqnum == packet.seqnum):
            # Create new message from packet and send it up to layer 5
            message = Msg(packet.payload)
            to_layer5(self, message)

            # Send response to EntityA saying we received the packet
            self.sndpkt = Pkt(packet.seqnum, self.expectedSeqnum, createCheckSum(packet.seqnum, self.expectedSeqnum, packet.payload), packet.payload)
            ...
            if(self.expectedSeqnum == self.seqnumLimit - 1):
                ...
                self.expectedSeqnum += 1
        # Else we received an out of order packet and we need to let Entity A know
        elif checkNotCorrupt(packet.checksum, packet.seqnum, packet.acknum, packet.payload):
            print("B - Received corrupted or incorrect packet sending ack to A")
            to_layer3(self, self.sndpkt)

    # Called when B's timer goes off.
    def timer_interrupt(self):
        # No Timer needed for part B
        pass
...
# Helper method for easier creation of checksums for packets


def createCheckSum(seqnum, acknum, bytes):
    return seqnum + acknum + int.from_bytes(bytes, "big")

# Helper method for easier check of corruption


def checkNotCorrupt(pktChecksum, pktSeqnum, pktAcknum, pktBytes):
    if pktChecksum == createCheckSum(pktSeqnum, pktAcknum, pktBytes):
        return True
    else:
        return False
...
class Simulator:
    ...
    def _insert_event(self, event):
        if self.trace > 2:
            ...
            print(
                f'            INSERTEVENT: future time will be {event.ev_time}')
